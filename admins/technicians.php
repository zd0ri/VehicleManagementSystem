    ...
    try {
        ...
        if ($action === 'edit') {
            if (!empty($_POST['password'])) {
                $password_hash = password_hash($_POST['password'], PASSWORD_DEFAULT);
                $stmt = $pdo->prepare("UPDATE users SET full_name = ?, email = ?, password_hash = ?, status = ? WHERE user_id = ? AND role = 'technician'");
                $stmt->execute([$_POST['full_name'], $_POST['email'], $password_hash, $_POST['status'], $_POST['user_id']]);
            } else {
...
$technicians = $pdo->query("SELECT * FROM users WHERE role = 'technician' ORDER BY created_at DESC")->fetchAll();
?>
<!DOCTYPE html>
<html lang="en">
<head>
            ...
            <div class="admin-card">
                ...
                <div class="table-responsive">
                    <table class="admin-table" id="techniciansTable">
                        <thead><tr><th>ID</th><th>Full Name</th><th>Email</th><th>Status</th><th>Created At</th><th>Actions</th></tr></thead>
                        <tbody>
                        ...
                        <tr>
                            ...
                            <td class="action-btns">
                                <button class="btn-icon btn-edit" onclick="editTech(<?= $row['user_id'] ?>, '<?= htmlspecialchars(addslashes($row['full_name']), ENT_QUOTES) ?>', '<?= htmlspecialchars(addslashes($row['email']), ENT_QUOTES) ?>', '<?= $row['status'] ?>')"><i class="fas fa-edit"></i></button>
...
function editTech(id,name,email,status){
